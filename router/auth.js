...
router.post('/api/images', upload.single('image'), async (req, res) => {
  ...
  const { name, email, phone, work, password, cpassword } = req.body;
    ...
    try{ 
      ...
      const userExist = await emplyeeSchema.findOne({ email: email });
...
router.post('/addproduct', upload.single('image'), async (req, res) => {
  ...
  const { name, type, core, weight, price, discount, special_discount, quantity } = req.body;
 
  if(!name || !type || !core || !weight || !price || !discount || !special_discount || !quantity)
  {
      console.log("please fill the field properly");
      return res.status(422).json({error: "please fill the field properly"});
  }
